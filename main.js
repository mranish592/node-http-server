...
function updateTodo(index, newTodoBody) {
  ...
  const result = todos.get(index);
  console.log(
    `updated :: new:${JSON.stringify(newTodo)}, old:${JSON.stringify(oldTodo)}`,
  );
  return result;
}
...
app.post("/update/:todoId", function (request, response) {
  const todoId = request.params.todoId;
...
app.get("/delete/:todoId", function (request, response) {
  const todoId = request.params.todoId;
